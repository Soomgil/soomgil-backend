...
class KtoPlaceDescriptionCacheTest {

	private final KtoPlaceDescriptionCache cache = new KtoPlaceDescriptionCache(
		new ConcurrentMapCacheManager(KtoPlaceDescriptionCache.CACHE_NAME)
	);

	@Test
	void reusesDescriptionOnlyWhileKtoModifiedTimeIsUnchanged() {
		...
		cache.put(original, "넓은 백사장이 있는 해수욕장");

		assertThat(cache.find(original)).contains("넓은 백사장이 있는 해수욕장");
