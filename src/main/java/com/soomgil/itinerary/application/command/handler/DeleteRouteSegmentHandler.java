...
/**
 * {@link DeleteRouteSegmentCommand}를 처리해 route segment를 soft delete한다.
 */
@Component
public class DeleteRouteSegmentHandler implements CommandHandler<DeleteRouteSegmentCommand, ItineraryMutationResult> {
	...
	public DeleteRouteSegmentHandler(
	...
	@Override
	@Transactional
	public ItineraryMutationResult handle(DeleteRouteSegmentCommand command) {
		...
		boolean deleted = repository.softDeleteRouteSegment(
			command.tripId(),
			command.routeId(),
			command.actorUserId(),
			now
