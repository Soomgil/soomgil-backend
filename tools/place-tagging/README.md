...
- 결과는 20곳(요청 1회)마다 `output/part-X/tagged.jsonl`에 바로 저장된다. 콘솔 내용은 `output/part-X/run.log`에도 남는다. **중간에 꺼져도 다시 더블클릭하면 이어서 한다.**
...
- 한 요청에 20곳을 보내므로 장소 수 ÷ 20 이 필요한 요청 수다. 같은 키를 두 컴퓨터가 쓰면 요청 수는 합산되니
  콘솔 잔량은 그 기준으로 본다.
